...
from clustering.modules.risk import compute_risk_score, generate_shap_plot
...
from .models import ClusteringJob, Dataset
from .pipeline import (execute_all_steps, execute_clustering,
                       execute_dimensionality_reduction,
                       execute_feature_selection, execute_metrics,
                       execute_preprocessing, execute_visualization)
from .presets import PRESETS
from .serializers import ClusteringJobSerializer, DatasetSerializer
...
class RiskShapExplanationView(APIView):
    def get(self, request, job_id, record_index):
        job = get_object_or_404(ClusteringJob, id=job_id)

        clustering_file = os.path.join(
            settings.MEDIA_ROOT, 'intermediate', str(job.id), 'clustering.csv'
        )
        if not os.path.exists(clustering_file):
            return Response(
                {"detail": "clustering.csv не найден. Выполните кластеризацию."},
                status=404
            )
        ...
        if idx < 0 or idx >= len(X):
            return Response(
                {"detail": f"Index {idx} вне диапазона (0–{len(X)-1})"},
                status=400
            )
        ...
        generate_shap_plot(model, data_row, feature_names=X.columns.tolist(), save_path=shap_path)

        shap_url = os.path.join(
            settings.MEDIA_URL, 'risk_explanations', str(job.id), shap_filename
        )
